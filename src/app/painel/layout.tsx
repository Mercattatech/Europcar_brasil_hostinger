...
const sidebarLinks = [
   ...
   { href: "/painel/jornada", label: "Jornada do Cliente", icon: "🗺️" },
...
// Quick access items for mobile bottom tab bar
const bottomTabItems = [
   { href: "/painel", label: "Home", icon: "📊" },
   { href: "/painel/reservas", label: "Reservas", icon: "📋" },
   { href: "/painel/relatorios", label: "Relatórios", icon: "📑" },
   { href: "/painel/jornada", label: "Jornada", icon: "🗺️" },
];
...
export default function PainelLayout({ children }: { children: React.ReactNode }) {
   ...
   if (!authorized) {
      return (
         <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
            <div className="bg-gray-900 border border-red-900/50 rounded-2xl p-8 md:p-10 text-center max-w-md w-full shadow-2xl">
               <div className="w-16 h-16 bg-red-600/20 rounded-full flex items-center justify-center mx-auto mb-6">
                  <svg className="w-8 h-8 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path>
                  </svg>
               </div>
               <h1 className="text-xl md:text-2xl font-black text-white mb-2">Acesso Negado</h1>
   ...
   return (
      <div className="flex h-[100dvh] bg-gray-950 overflow-hidden">
         {/* Desktop Sidebar */}
         <aside className={`hidden md:flex ${sidebarOpen ? 'w-64' : 'w-20'} bg-gray-900 border-r border-gray-800 flex-col transition-all duration-300 shrink-0`}>
            {/* Logo */}
         ...
         {mobileMenuOpen && (
            <div className="fixed inset-0 z-50 md:hidden">
               ...
               <div className="fixed top-0 left-0 w-[280px] h-full bg-gray-900 z-50 shadow-2xl flex flex-col">
                  {/* Header */}
                  <div className="h-14 flex items-center justify-between px-4 border-b border-gray-800">
                     <Link href="/" className="flex items-center gap-2">
                        <div className="bg-green-600 px-2 py-1 rounded">
                           <img src="/logo.jpg" alt="Europcar" className="h-5 object-contain" />
                        </div>
                     </Link>
                     <button onClick={() => setMobileMenuOpen(false)} className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-gray-800 text-gray-400">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
                     <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider px-3 mb-2">Menu</p>
                     {sidebarLinks.map(link => {
                        const isActive = pathname === link.href;
                        return (
                           <Link
                  ...
                  <div className="p-3 border-t border-gray-800">
                     ...
                     <button onClick={() => signOut({ callbackUrl: "/" })} className="w-full mt-2 text-xs font-bold text-gray-500 hover:text-red-400 py-2.5 rounded-lg hover:bg-gray-800 transition-all">
                        Sair da conta
                     </button>
                  </div>
               </div>
         ...
         <main className="flex-1 flex flex-col overflow-hidden">
            {/* Top Bar */}
            <header className="h-14 md:h-16 bg-gray-900/50 backdrop-blur-xl border-b border-gray-800 flex items-center justify-between px-4 md:px-6 shrink-0">
               <div className="flex items-center gap-3">
                  {/* Mobile hamburger */}
                  <button
                     onClick={() => setMobileMenuOpen(true)}
                     className="md:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-gray-800 text-gray-400"
                  >
                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" />
                     </svg>
                  </button>
                  <h1 className="text-white font-bold text-base md:text-lg">Painel Administrativo</h1>
                  <span className="text-[10px] bg-green-600/20 text-green-400 px-2 py-0.5 rounded-full font-bold uppercase hidden sm:inline">Admin</span>
               </div>
               <div className="flex items-center gap-4">
                  <Link href="/" className="text-xs md:text-sm text-gray-400 hover:text-white transition-colors font-medium" target="_blank">
                     Ver site →
                  </Link>
               </div>
            </header>

            {/* Page Content - with bottom padding for mobile bottom tab */}
            <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-gray-950 pb-20 md:pb-6">
               {children}
            </div>
         </main>

         {/* Mobile Bottom Tab Bar */}
         <div className="fixed bottom-0 left-0 right-0 md:hidden bg-gray-900/95 backdrop-blur-xl border-t border-gray-800 z-40 safe-bottom">
