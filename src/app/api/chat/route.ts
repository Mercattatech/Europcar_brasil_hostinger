...
export async function POST(req: Request) {
  ...
  // Load config from DB
  let config = null;
  let knowledgeDocs: { fileName: string; fileType: string; extractedText: string }[] = [];
  let approvedConversations: { messages: string }[] = [];

  try {
    config = await prisma.aIAgentConfig.findFirst();
    knowledgeDocs = await prisma.aIKnowledgeDoc.findMany({
      where: { active: true },
      select: { fileName: true, fileType: true, extractedText: true },
    });
    approvedConversations = await prisma.aIConversationLog.findMany({
      where: { approved: true },
      select: { messages: true },
      orderBy: { sessionStart: 'desc' },
      take: 10, // inject up to 10 approved conversations as examples
    });
  } catch (e) {
  ...
  const masterPrompt = config?.masterPrompt?.trim() || '';
...
══════════════════════════════════\n`;
  ...
  // Build few-shot examples from approved conversations
  let examplesSection = '';
  if (approvedConversations.length > 0) {
    examplesSection = `\n\nEXEMPLOS DE ATENDIMENTO APROVADOS (use como referência de tom e formato):\n`;
    for (const conv of approvedConversations) {
      try {
        const msgs = JSON.parse(conv.messages) as { role: string; content: string }[];
        const relevant = msgs.slice(0, 6); // use first 6 messages as example
        for (const m of relevant) {
          if (m.role === 'user') examplesSection += `Cliente: ${m.content}\n`;
          if (m.role === 'assistant') examplesSection += `Agente: ${m.content}\n`;
        }
        examplesSection += `---\n`;
      } catch {}
    }
  ...
  const systemPrompt = masterPrompt
    ? `${masterPrompt}${knowledgeSection}${examplesSection}`
...
${knowledgeSection}${examplesSection}`;
  ...
  const result = streamText({
    ...
    onFinish: async ({ text }) => {
      // Save/update conversation log for learning
      if (sessionId) {
        try {
          const allMessages = [
            ...messages,
